...
function To_do_list() {
    ...
    const addTask = () => {
        ...
        if (newTask) {
            ...
            editbutts.style = 'text-align: center;font-weight: 600; background: white;  display: flex;justify-content: center;align-items: center;width: 96px; height: 48px;border-radius: 6px; ';
            editinput.type = 'text';
            // p.htmlFor = "checkbox";
    ...
    return (
        <div id="to-do" className='md:w-[30rem] md:h-[25rem] h-[100vh] w-[100vw] bg-sky-500 grid-rows-[13%_10%_65%] md:grid-rows-[20%_10%_75%] items-center grid shadow-2xl shadow-blue-500/50'>
            <div className='flex md:gap-x-10 gap-x-5 items-center justify-center w-[100%] text-white'>
                <h1 className='md:text-3xl text-2xl font-bold'>To-Do list</h1>
                <div className=''>
                    <label className='md:text-2xl text-xl' htmlFor='date'>Date: </label>
            ...
            <div className='flex gap-x-5 justify-center px-4 ' id="task">
                ...
                <button id='butt' onClick={addTask} className='hover:bg-slate-300 flex justify-center items-center w-24 rounded-md h-12 bg-white font-[600] text-black'><p>Add</p></button>
            </div>

            <ul className='w-auto md:max-h-56 flex flex-col h-96 md:w-auto md:h-56 px-4 gap-y-3 overflow-y-auto task-list ' id='taskcontainer'></ul>
        </div>
    )
}
